...
<div align="center">
  <img src="docs/screenshots/hud-standby.png" alt="RON Holographic HUD Standby" width="900">
  <p><em>The Holographic Iron Man HUD — real-time telemetry, BST clock, 3-calendar temporal reckoning, Stark arc tachometer, and live activity feed.</em></p>
</div>
...
| 🛡️ **Cyber Watchdog & Net Radar** | Zero-GPU local Wi-Fi subnet radar and intruder auditor (`netradar.py`). Passive ARP harvesting, hardware OUI vendor resolution, rogue device alerts against `network_whitelist.json`, port security audit, and 360° circular sonar HUD radar (`Ctrl + Shift + N`). |
| 📑 **Document Intelligence & RAG** | Autonomous multi-format document parser (`docintel.py`) for PDF, DOCX, TXT, CSV, JSON with semantic chunking and TF-IDF caching. RAG document Q&A via HUD and Telegram (`/doc`), and table extraction with CSV export. |
| 🖼️ **Generative AI Art Studio** | Local and cloud text-to-image synthesis (`imagegen.py`) via Pollinations.ai, HuggingFace, Stability AI, and DALL-E 3 with automatic desktop wallpaper setting and Telegram mobile dispatch. |
| 🎯 **Daily Standup & AI Coach** | Morning goal-setting, interactive goal checklist, active window distraction blocker, focus analytics, and evening retrospective debrief (`Ctrl + Shift + C`). |
| 🧠 **Neural Long-Term Memory** | Autonomous fact retention (`memories.db`), semantic recall, and proactive context injection into LLM prompts without cloud dependencies. |
| 📻 **Live Intel Briefing Radio** | Multi-source streaming wire covering live crypto spot tickers, Hacker News discussion topics, GitHub trends, and global football scores with authentic radio-anchor broadcast cadence. |
| 🛰️ **Atmospheric Weather Station** | Multi-metric atmospheric telemetry with barometric pressure, UV index, cloud cover, and interactive 5-day predictive radar overlay (`#weather-station-overlay`). |
| 🌐 **Autonomous Web Autopilot** | Multi-step headless browser agent for shopping, comparison & search. Navigates, fills inputs, solves lazy loading, extracts tables, and streams live Base64 viewport to HUD |
| 💤 **Sleep & 'Hey Ron' Wake** | Voice-commanded sleep standby (*"Sleep"*, *"Go to sleep"*, *"ঘুমাও"*) with responsive "Hey Ron" / "Wake up" / "হে রন" wake-word detection and state restoration |
| 🔬 **Deep Researcher** | Multi-vector autonomous research: search, scrape, cross-reference, dual Markdown + PDF export in Documents, real-time HUD graph |
| 🌅 **Executive Briefing** | Start-of-day / end-of-day audio-visual briefing: weather, 3-calendar temporal coords, unread emails, battery & ping telemetry |
| ⚡ **Workspace Protocols** | One-command workstation orchestration (`work`, `gaming`, `lockdown`, `sleep`, `study`): apps, audio, power plan, HUD checklist |
| ✉️ **Email & Smart Reply** | IMAP unread email inspector and AI-drafted reply assistant |
| 🏎️ **Stark Tachometer** | Cyber speedometer with 240° arc gauge, spring-needle physics, and 3D warp conduit for internet speed tests |
...
| 🧭 **Browser Control** | DOM-first browsing: navigate, click, fill forms, read pages, extract data, save CSV/Excel, screenshots, downloads and explicitly authorised uploads |
...
| 🌐 **Generate Webpages** | *"Create a webpage for wishing happy birthday"* → a stylish, self-contained .html page with inline CSS + JS, opened in the browser — saved to Documents/Webpages |
...
| 🇧🇩 **Bangla Mode (বাংলা)** | *"turn on bangla mode"* → switches to Bangladeshi neural voice, Islamic greeting (*"আসসালামু আলাইকুম"*), and Emerald Green HUD theme |
| 🪟 **Floating Mini-HUD** | Frameless, transparent floating Arc Reactor orb on desktop with slide-out command bar via `Ctrl + Shift + Space` |
| 🔔 **System Tray Bar** | Sits in Windows taskbar tray with quick controls for visibility, mic mute, language, and Full HUD |
...
└───────────────────────────────────┬────────────────────────────────────┘
                         ...
                         └──┬──────────┬────────┘
                            │          │
                   (Direct) │          │ (Conversational / Function Calls)
                            ▼          ▼
             ┌───────────────────────────────┐
             │       LLM Function Caller     │
             │   (kat-coder via OpenAI API)  │
             └──────────────┬────────────────┘
                            ▼
 ┌────────────────────────────────────────────────────────────────────────┐
 │                           Feature Engines                              │
 │ • tribune.py       50+ source concurrent newspaper & 3-min audio radio │
 │ • coach.py         Daily standup, goal checklist & distraction blocker │
 │ • memory.py        Neural fact memory & proactive context injection    │
 │ • intel.py         Live intel radio, crypto tickers, tech & football   │
 │ • researcher.py    Autonomous multi-vector web search, scrape & report │
 │ • briefing.py      Morning / Evening executive briefings & telemetry   │
 │ • protocol.py      Work, Gaming, Lockdown, Sleep workstation automation│
 │ • tools.py         Web browsing, app launching, YouTube, PDF export    │
 │ • weather.py       Atmospheric weather station & live forecast radar   │
 │ • clock.py         BST clock & 3-calendar temporal coordinates         │
 │ • netspeed.py      Network latency, bandwidth & Stark Arc speedometer  │
 │ • finder.py        Bounded local filesystem search engine              │
 │ • volume.py        Windows Core Audio endpoint volume controller       │
 │ • email_notify.py  IMAP inbox scanner & SMTP AI reply assistant        │
 │ • telegram_bridge.py Remote smartphone bot, RON-Share & telemetry      │
 │ • docintel.py      Multi-format RAG, table extraction & document parser│
 │ • netradar.py      Subnet radar sweep, rogue MAC & port vulnerability  │
 │ • autopilot.py     Autonomous multi-step web agent & lazy-load scraper │
 │ • imagegen.py      Multimodal AI image generation & wallpaper studio   │
 └──────────────────────────┬─────────────────────────────────────────────┘
                            │
            ┌───────────────┴───────────────┐
            ▼                               ▼
 ┌──────────────────────┐        ┌──────────────────────┐
 │   voice.py (TTS)     │        │     bus.py (Event)   │
 │   Windows SAPI       │        │  PubSub Pipeline Bus │
 └──────────┬───────────┘        └──────────┬───────────┘
            ▼                               ▼
       🔊 Speaker                ┌──────────────────────┐
                                 │     ui_server.py     │
...
### 🖥️ Real-Time Holographic Interface States

| Standby / Idle | Voice Listening | LLM Reasoning |
| :---: | :---: | :---: |
| <img src="docs/screenshots/hud-standby.png" width="280" alt="Standby"> | <img src="docs/screenshots/hud-listening.png" width="280" alt="Listening"> | <img src="docs/screenshots/hud-thinking.png" width="280" alt="Thinking"> |
| **Tool Execution** | **Voice Speaking** | **System Diagnostics** |
| <img src="docs/screenshots/hud-executing.png" width="280" alt="Executing"> | <img src="docs/screenshots/hud-speaking.png" width="280" alt="Speaking"> | <img src="docs/screenshots/hud-error.png" width="280" alt="Diagnostics"> |
...
### 🌐 Autonomous Web Autopilot ("Browse and buy/find")
An autonomous multi-step headless browser agent that navigates complex websites, interacts with search fields, solves lazy loading/pagination, extracts structured data (prices, flights, issues), saves CSV reports, and streams a live base64 screenshot viewport directly into the holographic Web HUD.

| Say This | What Happens |
|---|---|
| *"Ron, check Daraz and Amazon and tell me the cheapest Logitech MX Master 3S"* | Searches products, extracts prices, ranks lowest first, streams live viewport, pops comparison table |
| *"Go to github and check the open issues on my RON repository"* | Navigates GitHub issues, extracts active bugs & PRs with links |
| *"Find the cheapest flight ticket from Dhaka to Bangkok next Friday on Skyscanner"* | Scrapes flight prices, synthesizes departure details, exports to CSV |
| *"দারাজে সবচেয়ে কম দামে ওয়্যারলেস মাউস খুঁজে বের করো"* | Bengali voice command: navigates, extracts prices, speaks debrief in Bengali |

### 🧭 Browser Control
Browser control keeps one Chrome session open and takes one observable action at
a time, allowing Ron to inspect the result before continuing.

Start a page with Ron (for example, *"Open wikipedia.org"*) before asking it to
read or control the page. This keeps the automation session separate from your
regular Brave windows and tabs.

For *"Read this page"*, Ron can also read the **foreground** Brave, Chrome,
Edge, or Firefox tab you opened yourself. Keep that tab active while speaking;
Ron copies its readable text and restores your text clipboard afterwards.

| Say This | What Happens |
|---|---|
| *"Read this page"* | Reads the visible content of the controlled browser tab |
| *"Click Sign in"* | Finds a control by CSS selector, visible text, or accessible label |
| *"Switch to Next Tab"* | Switched to next tab |
...
### 🔬 Autonomous Deep Research 2.0
| Say This | What Happens |
|---|---|
| *"Research the latest advancements in solid-state batteries in 2026"* | Launches multi-engine web search (ArXiv, Wikipedia, DDG), deep scraping, synthesis, dual Markdown+PDF export, and visualizes live on HUD |
| *"Quick research on RISC-V architecture"* | Rapid 3-vector executive scan with fast source synthesis |
| *"Exhaustive research on quantum computing error correction"* | Deep 7-vector academic+web harvest with gap-filling and comprehensive citation matrix |
| *"Compare Supabase vs Firebase pricing and scalability"* | Generates a structured side-by-side Comparative Matrix table on HUD and in dossier |
| *"Open research PDF"* / *"Show research report"* | Launches the generated research PDF in your default viewer |
| *"Read research summary"* | Speaks the executive research briefing and key takeaways |
| *"কোয়ান্টাম কম্পিউটিং নিয়ে ডিপ রিসার্চ করো"* | Autonomous deep research with full Bengali executive voice briefing |

See the **🔬 Autonomous Multi-Source Deep Researcher 2.0** section below for full details.

### ⚡ Work & Gaming Protocols
| Say This | What Happens |
|---|---|
| *"Initiate work protocol"* / *"Work mode"* | Opens VS Code & browser, sets balanced power plan, sets volume to 30%, launches checklist on HUD |
| *"Initiate gaming protocol"* / *"Gaming mode"* | Kills work apps, sets High Performance power plan, launches Steam/Discord, sets volume to 80% |
| *"Protocol zero"* / *"Lockdown protocol"* | Closes browser & distracting apps, mutes volume, sets power plan to Balanced |
| *"Sleep protocol"* | Closes heavy apps, mutes audio, prepares system for sleep |
| *"Study protocol"* | Launches research environment, sets volume to 20%, minimizes distractions |

See the **⚡ Work & Gaming Protocols** section below for full details.

### 🌅 Executive Briefings
| Say This | What Happens |
|---|---|
| *"Good morning, RON"* / *"Morning briefing"* | Spoken briefing with weather, rain outlook, 3 calendars, unread VIP emails, battery & network telemetry, and holographic dashboard cards (no video audio) |
| *"Evening briefing"* / *"Status report"* | End-of-day summary of system health, evening weather, remaining unread emails |

See the **🌅 Executive Morning & Evening Briefings** section below for full details.

### ✉️ Email & Smart Reply
| Say This | What Happens |
|---|---|
| *"Check my unread emails"* / *"Read unread emails"* | Connects via secure IMAP, reads recent unread subject lines & senders, displays on HUD |
| *"Reply to [sender] saying [message]"* | AI drafts and sends professional email response via SMTP |

### 📡 Internet Speed
| Say This | What Happens |
|---|---|
| *"Check my internet speed"* / *"Run speed test"* | Live latency ping, download & upload bandwidth test with Stark Arc Tachometer on HUD |

### ⏱️ Timer
| Say This | What Happens |
|---|---|
| *"Set a timer for 5 minutes"* | Starts countdown timer with live circular progress bar on HUD |
| *"Cancel timer"* / *"Stop timer"* | Aborts active timer |

### 🔊 Volume Control
| Say This | What Happens |
|---|---|
| *"Set volume to 50"* / *"Volume 75%"* | Adjusts Windows master volume via Core Audio |
| *"Mute"* / *"Unmute"* | Toggles master audio mute state |
| *"Volume up 10"* / *"Volume down 20"* | Incremental volume adjustments |

### 📰 The RON World Tribune (Personal AI Newspaper Editor)
An autonomous, publication-grade daily newspaper compiled and designed for you every morning with **zero GPU overhead** (100% CPU thread-pool harvesting and Chromium headless print). Concurrently scrapes 50+ wire services across 6 news desks: World & Geopolitics, Technology & Frontier AI, Financial Markets & Crypto, Science & Aerospace, Global Football, and Bangladesh Bureau.

<div align="center">
  <img src="docs/screenshots/hud-tribune.png" alt="The RON World Tribune Broadsheet Reader" width="850">
  <p><em>The RON World Tribune Broadsheet Reader — 50+ source concurrent harvesting, 4-quadrant executive summary, and multi-column article grids.</em></p>
</div>

| Say This | What Happens |
|---|---|
| *"Show newspaper"* / *"Open newspaper"* / *"Newspaper"* | Opens the holographic broadsheet HUD modal reader (`Ctrl + Shift + T`) |
| *"Ron, read me today's Tribune"* / *"Listen to Tribune broadcast"* | Delivers a 3-minute spoken executive audio radio broadcast with anchor cadence |
| *"Open newspaper PDF"* / *"Show newspaper PDF"* | Automatically compiles and launches the Victorian/Modern broadsheet PDF in Documents |
| *"Generate today's newspaper"* / *"Publish Tribune"* | Force re-scrapes all 50+ sources and publishes a fresh edition |
| *"আজকের ট্রাইবিউন পড়"* | Spoken 3-minute executive broadcast in natural Bengali |
| *"সংবাদপত্র দেখাও"* | Opens the broadsheet HUD overlay via Bengali voice command |
| *"সংবাদপত্র পিডিএফ"* | Launches the generated PDF broadsheet directly |

### 🎯 Daily Standup & AI Focus Coach
An autonomous executive coach and daily standup manager. Ingests morning priorities, maintains an active goal checklist, tracks focus metrics, blocks distracting applications in real-time, and debriefs your day in the evening.

<div align="center">
  <img src="docs/screenshots/hud-coach.png" alt="Daily Standup & AI Focus Coach" width="850">
  <p><em>Daily Standup Console — Morning priority checklist, active distraction blocker, and evening retrospective debrief.</em></p>
</div>

| Say This | What Happens |
|---|---|
| *"Start standup"* / *"Daily standup"* | Opens the Standup modal (`Ctrl + Shift + C`) and initiates goal review |
| *"Set my goals today: finish testing, write docs, review PR"* | Ingests and numbers your daily goals into the live checklist |
| *"What are my goals today"* / *"Check standup"* | Speaks current goal status and shows pending tasks |
| *"Mark goal 1 as done"* / *"I finished writing documentation"* | Ticks off goal, plays milestone audio cue, and updates progress bar |
| *"Enable distraction blocker"* / *"Turn on focus mode"* | Activates active-window monitor; minimizes distracting apps during work |
| *"Turn off distraction blocker"* | Restores normal desktop application access |
| *"Evening debrief"* / *"Wrap up the day"* | Conducts an end-of-day retrospective and calculates focus score |

### 🧠 Neural Long-Term Memory
Local, private SQLite-backed memory store (`memories.db`). Retains personal facts, preferences, dates, and credentials across reboots without cloud storage. Proactively injects relevant contextual facts into LLM system prompts so Ron knows your preferences automatically.

| Say This | What Happens |
|---|---|
| *"Remember that my mother's birthday is October 14"* | Saves the fact permanently to local SQLite memory store |
| *"Remember that my favourite programming language is Python"* | Saves user preference for proactive LLM guidance |
| *"What do you remember about my mother"* | Queries memories and speaks the answer |
| *"Forget my mother's birthday"* | Removes the specified memory securely |
| *"Show my memories"* / *"Show memories"* | Opens the Memory Console on the HUD overlay |

### 📻 Live Intel Briefing Radio ("RON World Report")
A multi-stream intelligence wire with real-time audio broadcasting. Fetches live spot crypto tickers, Hacker News front page, tech headlines, trending GitHub repositories, and global football scores.

<div align="center">
  <img src="docs/screenshots/hud-intel.png" alt="Live Intel Briefing Radio" width="850">
  <p><em>RON World Report Intel Console — Streaming global wire, crypto tickers, Hacker News, and GitHub trending.</em></p>
</div>

<div align="center">
  <img src="docs/screenshots/hud-football.png" alt="Global Sports & Football Wire" width="850">
  <p><em>Global Sports & Football Telemetry — Live fixtures, match results, and club dispatches.</em></p>
</div>

| Say This | What Happens |
|---|---|
| *"Give me the morning intel briefing"* / *"World report"* | Plays futuristic radio chime and broadcasts full multi-desk intel audio |
| *"Read all sports news"* / *"Football scores"* | Summarizes latest fixtures, scores, and transfers |
| *"How did Real Madrid play"* / *"Barcelona score"* | Club-specific match scores and recap |
| *"What's happening in tech"* | Speaks Hacker News & tech headlines |
| *"Check crypto markets"* / *"Market briefing"* | Live Bitcoin & Ethereum prices with percentage movements |
| *"Show trending GitHub repos"* | Speaks top trending developer repositories |
| *"Open intel HUD"* / *"Show world report"* | Opens the interactive Live Intel overlay on the HUD |

### 🛰️ Atmospheric Weather Station & Radar
An advanced meteorological console displaying live barometric pressure, UV index, cloud cover, humidity, wind velocity, and 5-day predictive radar.

<div align="center">
  <img src="docs/screenshots/hud-weather-station.png" alt="Atmospheric Weather Station & Radar" width="850">
  <p><em>Atmospheric Weather Station Console — Barometric pressure, UV index, humidity, wind velocity, and 5-day predictive radar.</em></p>
</div>

| Say This | What Happens |
|---|---|
| *"Open weather station"* / *"Weather station"* | Opens the full Atmospheric Weather Station overlay (`#weather-station-overlay`) |
| *"Show weather station in Tokyo"* | Fetches telemetry and 5-day forecast for any global city |

### 📱 RON Pocket Uplink (Two-Way Remote Command Center via Telegram)
Stay connected with your workstation from anywhere in the world. Using a private, secured Telegram bot paired exclusively to your Telegram account, you can send voice notes or text commands from your smartphone (e.g., while in transit, at a café, or at university) and have RON execute them autonomously on your desktop.

#### 🌟 Key Capabilities
- **📊 Real-Time Telemetry:** Send `/status` or *"status report"* to receive live CPU %, RAM usage, battery level, uptime, active foreground window, and HUD connection state.
- **📸 Remote Desktop Screenshot:** Send `/screenshot` or *"send screenshot"* to capture the active desktop and upload the high-res image directly into your Telegram chat.
- **🔬 Autonomous Deep Research & PDF Delivery:** Send `/research <topic>` (e.g. `/research solid-state batteries in 2026`). RON scouts the web, synthesizes sources, generates a publication-grade PDF dossier, and uploads it directly to your phone.
- **📰 Today's Newspaper on Mobile:** Send `/tribune` or *"send newspaper"* to receive *The RON World Tribune* broadsheet PDF on your phone.
- **🔒 Emergency Lockout:** Send `/lock` to instantly call Windows `LockWorkStation()` and secure your desktop remotely.
- **🌦️ Atmospheric Conditions & Standup Goals:** Send `/weather` or `/standup` to monitor local conditions or check your active daily priorities.
- **💬 Conversational AI & Tool Execution:** Ask questions, perform calculations, or trigger any tool in natural language directly through the chat.
- **🛡️ Ironclad Security & Pairing:** Unauthorized users are instantly rejected with security alerts dispatched to the desktop HUD. Pair your phone in seconds by sending `/pair <PIN>` (default: `/pair RON-7701`, configurable in `config.json`).

#### ⚡ Quick Setup
1. Message [`@BotFather`](https://t.me/BotFather) on Telegram and create a new bot with `/newbot`.
2. Copy your bot token into `config.json`:
   ```json
   "telegram": {
     "enabled": true,
   ...
   ```
   *(Or set the environment variable: `set TELEGRAM_BOT_TOKEN=YOUR_BOT_TOKEN`)*
3. Start RON (`python main.py` or `python ui_server.py`). The top-right HUD bar will display the **`UPLINK`** status chip.
4. Open your bot in Telegram and send `/pair RON-7701`. Your device is now authorized and paired!

#### 📁 RON-Share: Mobile Drop & Remote Repository
Wirelessly beam files, documents, photos, audio clips, and APKs directly between your smartphone and your desktop workstation:
- **Instant Drop**: Send any file, document, or photo to your paired Telegram bot. RON automatically downloads it into `Documents\RON-Share\` and logs it in the repository index.
- **Repository Telemetry**: Send `/share` or `\share` to check total files stored, storage footprint, and repository path.
- **File Catalog**: Send `/share list` to inspect the 15 most recently transferred files with item IDs and file sizes.
- **Remote File Retrieval**: Send `/share get <id>` to have RON upload that file from your desktop directly back to your smartphone.
- **Open On Desktop**: Send `/share open` on Telegram or say *"open shared folder"* / *"open ron share"* / *"share folder"* to instantly launch Windows Explorer to `Documents\RON-Share`.
- **Document Q&A (RAG)**: Send `/doc <question>` to query and extract insights from documents in your repository.

### 🛡️ Network Radar & Hacker Intrusion Scanner ("RON Cyber Watchdog")
A real-time local Wi-Fi subnet radar and workstation vulnerability auditor operating with **zero GPU overhead and 100% CPU safety**. Built entirely on Python standard library socket sweeps and Windows ARP cache harvesting without requiring WinPcap or Npcap drivers.

| Say This | What Happens |
|---|---|
| *"Scan network"* / *"Scan Wi-Fi"* / *"Network radar"* | Sweeps the active `/24` subnet, detects active devices, checks open ports, and opens the holographic Sonar Radar modal (`Ctrl + Shift + N`) |
| *"Check network security"* / *"Intrusion scan"* | Audits connected devices against trusted whitelist and flags unauthorized rogue MACs |
| *"হোম নেটওয়ার্ক স্ক্যান করো"* | Bengali voice command: scans Wi-Fi subnet and provides tactical Bengali voice debrief |

#### 🌟 Key Capabilities
- **🌐 360° Circular Sonar Sweep Canvas:** Displays real-time rotating phosphor sonar beam, range rings, and node blips corresponding to active devices on your home network.
- **🏷️ Hardware OUI Identification:** Automatically resolves hardware manufacturers (Apple, Samsung, Intel, Espressif, Raspberry Pi, TP-Link, Xiaomi, Google, Sony PlayStation, Microsoft Xbox).
- **🚨 Rogue Device Intrusion Detection:** Compares active MAC addresses against `network_whitelist.json`. Alerts with visual HUD warnings and voice debriefs whenever an unrecognized IoT device or unauthorized node joins your network.
...
## 🔬 Autonomous Multi-Source Deep Researcher 2.0

Ron features an advanced autonomous multi-engine deep research intelligence agent capable of investigating technical topics, competitive landscapes, or complex engineering problems with academic rigor.

### The Research Lifecycle 2.0

```
[Voice / HUD Query] ──► Multi-Vector Query Planning (Hypothesis & 3-7 Vectors)
                                    │
                                    ▼
       Multi-Engine Harvesting (DuckDuckGo + Wikipedia + ArXiv + GitHub)
                                    │
                                    ▼
       Concurrent High-Speed Scraping (ThreadPoolExecutor) & Trust Scoring
                                    │
                                    ▼
          Reflective Gap Filling (Benchmarking & Pricing Validation)
                                    │
                                    ▼
           Executive Synthesis (Comparison Matrix + Numbered Citations)
                                    │
            ┌───────────────────────┴───────────────────────┐
            ▼                                               ▼
Dual Export & History Archive                  HUD Holographic Console 2.0
• Markdown (`Documents/Research_*.md`)         • Live Telemetry & Stepper
• Publication PDF (`Documents/Research_*.pdf`) • In-HUD Dossier Reader Tab
• Persistent `research_history.json`           • Comparison Matrix Tab & Archive
```

1. **Multi-Vector Planning**: Generates 3 to 7 focused vectors based on depth (`quick`, `deep`, `exhaustive`).
2. **Multi-Engine Harvesting**: Combines DuckDuckGo (`ddgs`), Wikipedia REST API (definitions & history), ArXiv API (scientific papers with authors & dates), and GitHub Search API (developer codebases).
3. **High-Speed Concurrent Scraping**: Scrapes all candidate sources in parallel with `ThreadPoolExecutor`, stripping clutter and computing algorithmic **Source Trust Scores** (Academic: 98%, Official Docs: 92%, Press: 85%, Web: 74%).
4. **Reflective Gap Filling**: Evaluates harvested data for missing quantitative benchmarks or pricing, dynamically executing secondary searches to fill factual gaps.
5. **Publication-Grade Synthesis**: Produces executive dossiers with structured **Comparative Matrices**, deep technical breakdowns, trade-offs, and verified citations tables with inline `[1]`, `[2]` links.
6. **HUD Console 2.0**: Features a live **In-HUD Dossier Reader**, **Comparison Matrix tab**, **Research Archive browser**, depth toggles, and quick action buttons (`OPEN PDF`, `DOCS FOLDER`, `COPY REPORT`, `LISTEN BRIEFING`).
7. **Voice Actions**: Open PDFs or read briefings hands-free (*"Open research PDF"*, *"Read research summary"*).

### Trigger Phrases
```bash
python main.py "Research the latest advancements in solid-state batteries in 2026"
python main.py "Quick research on RISC-V microcontrollers"
python main.py "Exhaustive research on graph neural networks"
python main.py "Compare Supabase vs Firebase pricing and scalability"
python main.py "Open research PDF"
python main.py "Read research summary"
```

### Verification
```bash
python -m unittest test_researcher.py   # 16 unit & API tests (multi-engine, concurrent scraping, archive)
```

---

## 🌅 Executive Morning & Evening Briefings

Start or conclude your day with an audio-visual briefing inspired by J.A.R.V.I.S. (*"Good morning, RON"*).

### What Gets Briefed

- **🌦️ Atmospheric Conditions**: Live temperature, apparent temperature, and precipitation chance for Bangladesh / configured city with umbrella recommendations.
- **🕰️ Temporal Coordinates**: Accurate synchronized reckoning across **Gregorian (English)**, **Bangla (Bangla Academy)**, and **Hijri (Islamic)** calendars.
- **✉️ Inbox Triage**: Rapid check of unread priority messages and VIP senders via IMAP.
- **🔋 System Vitals**: Hardware battery percentage, AC power status, and network ping latency.
- **🔇 Audio-First & Non-Intrusive**: Deliverable strictly via spoken voice without intrusive video playback.
- **🪟 Holographic HUD Dashboard**: Renders `#briefing-overlay` featuring high-contrast neon weather cards, calendar tags, email counts, and hardware telemetry gauges.

### Trigger Phrases
```bash
python main.py "Good morning, RON"
python main.py "Morning briefing"
python main.py "Evening briefing"
python main.py "Status report"
```

### Verification
```bash
python test_briefing.py          # 12 offline unit tests (stubbed telemetry & mail)
```

---

## ⚡ Work & Gaming Protocols (Workspace Automation)

One-command workstation orchestration that transitions your environment between intense productivity, immersive gaming, focused study, and rest.

### Available Protocols

| Protocol | Power Plan | Master Volume | Applications Launched | Processes Terminated / Actions |
|---|---|---|---|---|
| **Work (`work`)** | Balanced | 30% | VS Code, Chrome / Browser | Distraction audit, cleans background tasks |
| **Gaming (`gaming`)** | High Performance | 80% | Steam, Discord | Closes productivity apps (Notepad, VS Code) |
| **Protocol Zero / Lockdown (`lockdown` / `zero`)** | Balanced | Muted (0%) | None | Closes browsers, kills distractors, locks workstation down |
| **Sleep (`sleep`)** | Power Saver / Balanced | Muted (0%) | None | Closes heavy apps, dims HUD, prepares system for sleep |
| **Study (`study`)** | Balanced | 20% | Research tools, Notepad | Minimizes distractions, enables focus mode |

### On the HUD
The `#protocol-overlay` projects an Iron Man-style execution checklist on your screen, marking each action (power scheme switch, audio calibration, app launch, process termination) with animated status indicators.

### Trigger Phrases
```bash
python main.py "Initiate work protocol"
python main.py "Gaming protocol"
python main.py "Protocol zero"
python main.py "Sleep protocol"
```

### Verification
```bash
python test_protocol.py          # 13 offline unit tests (stubbed COM audio & powercfg)
```

---

## ✉️ Email Inspection & AI Compose/Reply Assistant

Ron connects securely with your email provider via IMAP and SMTP to give you hands-free voice triage over your inbox.

- **Inbox Inspection**: Reads unread senders, subject lines, and timestamps without marking messages as read.
- **Smart AI Composition & Reply**: Dictate your intent in plain English (e.g. *"Reply to Thanos saying thank you and confirming the Monday meeting"* or *"Send a birthday wish email to Thanos"*). Ron dynamically invokes the LLM (`EMAIL_COMPOSE_PROMPT`) to compose an authentic, context-aware email with proper salutation, body paragraphs, and professional signature, and confirms with the user before dispatching via authenticated SMTP.
- **Contact Book Resolution**: Matches spoken recipient names (e.g., *"Thanos"*, *"Boss"*, *"Dad"*) to real email addresses defined in `contacts.json`.

### Trigger Phrases
```bash
python main.py "Check my unread emails"
python main.py "Read unread emails"
python main.py "Reply to Thanos saying thank you"
python main.py "Send a birthday wish email to Thanos"
```

### Verification
```bash
python test_email.py             # 35 offline unit tests (IMAP/SMTP mocks & LLM compose prompt)
python test_email_reading.py     # 12 offline reading & triage tests
```
...
├── telegram_bridge.py   # 📱 Telegram Pocket Uplink & RON-Share repository
├── researcher.py        # 🔬 Autonomous multi-source deep researcher & PDF exporter
├── docintel.py          # 📑 Multi-format document parser, table extractor & local RAG
├── netradar.py          # 🛡️ Cyber Watchdog local Wi-Fi subnet radar & port auditor
├── autopilot.py         # 🌐 Multi-step headless browser agent with live HUD feed
├── browser_agent.py     # 🧭 Lightweight DOM browser assistant
├── imagegen.py          # 🖼️ Multimodal AI image generation & wallpaper studio
├── tribune.py           # 📰 Daily AI newspaper editor & 3-min executive radio
├── coach.py             # 🎯 Daily standup, goal tracker & distraction blocker
├── memory.py            # 🧠 Neural long-term fact memory & proactive context
├── intel.py             # 📻 Live streaming wire (crypto, tech, football)
├── briefing.py          # 🌅 Executive morning & evening briefings & telemetry
├── protocol.py          # ⚡ Work, Gaming, Lockdown, Sleep workstation automation
├── email_notify.py      # ✉️ IMAP inbox triage & SMTP AI-drafted reply engine
...
├── memories.db          # 🧠 SQLite database for long-term user memories
...
├── mini_hud.py          # 🪟 Floating transparent Arc Reactor desktop widget
├── tray.py              # 🔔 Windows system tray icon & control menu
├── startup_manager.py   # ⚙️ Windows startup registry manager
├── ui_server.py         # 🖥️ HUD web server + SSE bridge
├── ui/                  # 🎨 Holographic interface (index.html, app.js, style.css)
├── config.json          # ⚙️ Central configuration (APIs, Telegram, email, radar)
├── contacts.json        # 📇 Email address book for named contacts
├── coach.json           # 📋 Standup goals & active priorities cache
├── network_whitelist.json # 🛡️ Trusted MAC address whitelist for intrusion scanner
├── requirements.txt     # 📦 Python dependencies
├── run_ron.bat          # 🪟 Windows launcher script
├── run_ron_ui.bat       # 🪟 Launcher for the HUD
├── run_mini_hud.bat     # 🪟 Launcher for floating desktop orb
├── test_autopilot.py    # 🌐 Offline browser autopilot tests
├── test_briefing.py     # 🌅 Offline executive briefing check (stubbed telemetry & weather)
├── test_browser_agent.py# 🧭 Offline browser agent checks
├── test_clock.py        # 🕰️ Offline clock & calendar check (no mic/API needed)
├── test_coach.py        # 🎯 Offline daily standup & coach tests
├── test_email.py        # ✉️ Offline email IMAP/SMTP mocks & LLM generation tests
├── test_email_reading.py# ✉️ Offline email reading and triage tests
...
├── test_imagegen.py     # 🖼️ Offline AI image generation tests
├── test_intel.py        # 📻 Offline intel briefing radio tests
├── test_memory.py       # 🧠 Offline neural memory & fact extraction tests
├── test_mic.py          # 🎤 Microphone discovery & test utility
├── test_mini_hud.py     # 🪟 Offline floating mini-HUD tests
├── test_netspeed.py     # 📡 Offline speed-test check (stubbed endpoints, no network)
├── test_open_website.py # 🌐 Offline URL resolver tests
├── test_pdf.py          # 📄 Offline PDF renderer check (no mic/API needed)
├── test_protocol.py     # ⚡ Offline protocol execution check (stubbed volume & powercfg)
├── test_researcher.py   # 🔬 Offline deep research pipeline check (stubbed search & LLM)
├── test_share.py        # 📁 Offline RON-Share repository tests
├── test_sleep_wake.py   # 💤 Offline sleep & wake detection tests
├── test_timer.py        # ⏱️ Offline timer check (stubbed bus, no real countdown)
├── test_tribune.py      # 📰 Offline World Tribune newspaper compilation tests
├── test_ui_render.py    # 🖼️ Headless render check for the interface
├── test_volume.py       # 🔊 Offline volume check (GUID layout, phrasing, COM path)
...
├── docs/                # 📚 Feature guides and setup docs
│   ├── ADDING_FEATURES.md   # 🛠️  How to add a new tool / feature
│   └── IMAGE_GEN_SETUP.md   # 🎨  Image generation provider switching
...
- [x] GUI overlay with live waveform visualization & Stark Arc tachometer
- [x] Three-calendar temporal reckoning (English, Bangla, Hijri)
- [x] Email triage (IMAP) and AI-drafted reply integration (SMTP)
- [x] Work & gaming workspace protocol automations
- [x] Autonomous multi-source deep researcher with dual Markdown/PDF export
- [x] Autonomous headless web autopilot with live Base64 viewport streaming
- [x] Low-power sleep standby & wake-word detection (*"Hey Ron"* / *"Wake up"*)
- [x] Neural persistent long-term memory (`memories.db`) with proactive LLM context injection
- [x] Daily Standup & AI Executive Coach with active distraction blocker
...
See **[docs/ADDING_FEATURES.md](docs/ADDING_FEATURES.md)** for the exact steps
and a worked example. The short version:

1. Add a module (`your_feature.py`) with `run()`, `hud_payload()` and `describe()`
2. Add the tool schema to `SYSTEM_PROMPT` in `main.py`
3. Add an `extract_*()` direct-route matcher so common phrasings cost zero tokens
4. Add a `handle_get_*()` LLM fallback and wire both into `_process_command()`
5. Add a bus snapshot key in `bus.py` if the feature needs a HUD panel
6. Add an offline test (`test_your_feature.py`) and a docs page in `docs/`
