...
def check_history(port):
    """Hit /api/history directly. Returns (status, turn_count, more)."""
    url = f"http://127.0.0.1:{port}/api/history?limit=5"
    try:
        with urllib.request.urlopen(url, timeout=5) as res:
            body = json.loads(res.read().decode("utf-8"))
            return res.status, len(body.get("turns", [])), bool(body.get("more"))
    except urllib.error.HTTPError as e:
        return e.code, 0, False
    except Exception as e:
        print(f"[/api/history unreachable: {e}]")
        return 0, 0, False
...
def render(port, size, state, out):
    ...
    try:
        ...
        # Exercise the scrollback path explicitly rather than waiting for a
        # scroll gesture: a fault in it surfaces as a console error, which this
        # check already treats as a failure.
        driver.execute_script("scrollback.more = true; loadOlder();")
        time.sleep(1.0)
        ...
        report = driver.execute_script("""
          ...
          return {
            ...
            turns: q('#conversation').querySelectorAll('.turn').length,
            pastTurns: q('#conversation').querySelectorAll('.turn.past').length,
            scrollbackMore: scrollback.more,
            scrollbackMarker: (q('#conversation .scrollback') || {}).textContent || '',
            conversationEmpty: !!q('#conversation .empty'),
...
def main():
    ...
    status, stored_turns, _ = check_history(args.port)
    report["historyStatus"] = status
    report["historyTurns"] = stored_turns
    ...
    if report["historyStatus"] != 200:
        problems.append(f"/api/history returned {report['historyStatus']}")
    # A database with rows but a conversation panel with none means the
    # scrollback fetch ran and dropped what it got on the floor.
    if report["historyTurns"] and not report["turns"]:
        problems.append(f"{report['historyTurns']} stored turn(s) but the "
